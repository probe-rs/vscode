...
export class LiveWatchValueEditor {
    static async editVariableValue(variable: LiveWatchVariable) {
        ...
        if (newValue !== undefined) {
            ...
            if (success) {
                vscode.window.showInformationMessage(`Successfully updated ${variable.label} to ${newValue}`);
                // The variable value should already be updated via the setVariableValue method
            } else {
                vscode.window.showErrorMessage(`Failed to update ${variable.label}. Check that the variable is modifiable and in scope.`);
            }
        }
    }
}
...
export class LiveWatchDragAndDropController implements vscode.TreeDragAndDropController<vscode.TreeItem> {
...
export class LiveWatchValueRenameProvider implements vscode.ReferenceProvider, vscode.DefinitionProvider {

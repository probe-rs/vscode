...
export class ConditionalWatchManager {
    static async addConditionalWatch(variable: LiveWatchVariable) {
        ...
        if (condition) {
            ...
            vscode.window.showInformationMessage(`Conditional watch added: ${variable.label} will update only when ${condition}`);
        }
    }
    ...
    static removeConditionalWatch(variable: LiveWatchVariable) {
        ...
        if (variable.getConditionalWatch()) {
            ...
            vscode.window.showInformationMessage(`Conditional watch removed from ${variable.label}`);
        } else {
            vscode.window.showInformationMessage(`${variable.label} does not have a conditional watch`);
        }
    }
    ...
    static async editConditionalWatch(variable: LiveWatchVariable) {
        ...
        if (!currentConditionalWatch) {
            vscode.window.showInformationMessage(`${variable.label} does not have a conditional watch to edit`);
            return;
        }
        ...
        if (newCondition) {
            ...
            vscode.window.showInformationMessage(`Conditional watch updated: ${variable.label} will update only when ${newCondition}`);
        }
    }
}
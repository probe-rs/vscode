export class DataTypeFormatter {
    static formatValue(value: string, type?: string, format?: 'auto' | 'decimal' | 'hex' | 'binary' | 'float'): string {
        ...
        if (format === 'auto' && type) {
            if (type.toLowerCase().includes('float') || type.toLowerCase().includes('double')) {
                format = 'float';
            } else if (type.toLowerCase().includes('int') || type.toLowerCase().includes('char') || type.toLowerCase().includes('bool')) {

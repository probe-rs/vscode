...
function handleExit(code: number | null, signal: string | null) {
    ...
    if (code) {
        vscode.window.showErrorMessage(
            'ERROR: `probe-rs-debugger` exited with an unexpected code: '
                .concat(`${code}`)
                .concat(actionHint),
        );
    } else if (signal) {
        vscode.window.showErrorMessage(
            'ERROR: `probe-rs-debugger` exited with signal: '
                .concat(`${signal}`)
                .concat(actionHint),
        );
    }
    ...
    if (fromDebugger) {
        ...
        switch (probeRsLogLevel) {
            ...
            case 'Info': // Log Info, Warn AND Error
                if (
                    consoleMesssage.includes('INFO') ||
                    consoleMesssage.includes('WARN') ||
                    consoleMesssage.includes('ERROR')
                ) {
                    vscode.debug.activeDebugConsole.appendLine(consoleMesssage);
                }
                break;
            case 'Debug': //  Log Info, Warn, Error AND Debug
                if (
                    ...
                    consoleMesssage.includes('WARN') ||
                    consoleMesssage.includes('ERROR') ||
                    consoleMesssage.includes('DEBUG')
                ) {
                    vscode.debug.activeDebugConsole.appendLine(consoleMesssage);
                }
                break;
...
class ProbeRSDebugAdapterServerDescriptorFactory implements vscode.DebugAdapterDescriptorFactory {
    ...
    createRttTerminal(channelNumber: number, dataFormat: string, channelName: string) {
        ...
        if (vscode.debug.activeDebugSession) {
            ...
            if (
                session.configuration.hasOwnProperty('rttEnabled') &&
                ...
                let channelPty: vscode.Pseudoterminal = {
                    ...
                    open: () => {
                        ...
                        session
                            ...
                            .then((response) => {
                                logToConsole(
                                    'DEBUG: probe-rs: RTT Window opened, and ready to receive RTT data on channel' +
                                        JSON.stringify(channelNumber, null, 2),
                                );
                            });
                    },
                    close: () => {
                        ...
                        session
                            ...
                            .then((response) => {
                                logToConsole(
                                    'DEBUG: probe-rs: RTT Window closed, and can no longer receive RTT data on channel' +
                                        JSON.stringify(channelNumber, null, 2),
                                );
                            });
                    },
                ...
                for (let reuseTerminal of vscode.window.terminals) {
                    if (reuseTerminal.name === channelName) {
                        ...
                        session
                            ...
                            .then((response) => {
                                logToConsole(
                                    'DEBUG: probe-rs: RTT Window reused, and ready to receive RTT data on channel' +
                                        JSON.stringify(channelNumber, null, 2),
                                );
                            });
                        break;
                ...
                if (channelTerminal === undefined) {
                    ...
                    vscode.debug.activeDebugConsole.appendLine(
                        'probe-rs-debugger: Opened a new RTT Terminal window named: ' + channelName,
                    );
        ...
        switch (customEvent.event) {
            ...
            case 'probe-rs-show-message':
                switch (customEvent.body?.severity) {
                    ...
                    default:
                        logToConsole(
                            'ERROR: prober-rs: Received custom event with unknown message severity: \n' +
            ...
            default:
                logToConsole(
                    'ERROR: probe-rs: Received unknown custom event:\n' +
    ...
    async createDebugAdapterDescriptor(
        ...
        if (session.configuration.hasOwnProperty('server')) {
            ...
            logToConsole(
                'INFO: Debug using existing server' +
                    JSON.stringify(debugServer[0]) +
                    ' on port ' +
                    JSON.stringify(debugServer[1]),
            );
            ...
            try {
                ...
                vscode.window.showErrorMessage(
                    'Searching for available port failed with: ' +
            ...
            var debuggerReadySignature =
                'CONSOLE: Listening for requests on port ' + debugServer[1];
...
class ProbeRsDebugAdapterTrackerFactory implements DebugAdapterTrackerFactory {
    createDebugAdapterTracker(
        ...
        logToConsole('DEBUG: Creating new debug adapter tracker');
...
class ProbeRsDebugAdapterTracker implements DebugAdapterTracker {
    onWillReceiveMessage(message: any) {
        if (probeRsLogLevel === 'Debug' || probeRsLogLevel === 'Trace') {
            logToConsole(
                'DEBUG: Received message from debug adapter:\n' + JSON.stringify(message, null, 2),
            );
        }
    }
    ...
    onDidSendMessage(message: any) {
        if (probeRsLogLevel === 'Debug' || probeRsLogLevel === 'Trace') {
            logToConsole(
                'DEBUG: Sending message to debug adapter:\n' + JSON.stringify(message, null, 2),
            );
        }
    }
    ...
    onError(error: Error) {
        if (probeRsLogLevel === 'Debug' || probeRsLogLevel === 'Trace') {
            logToConsole(
                'ERROR: Error in communication with debug adapter:\n' +
